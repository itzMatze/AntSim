...
{
	return hash_jenkins32(uint(index.x)) ^ hash_jenkins32(uint(index.y));
}
...
int try_acquire_hash_grid_cell_index(ivec2 index)
{
	...
	for (int bucket_offset = 0; bucket_offset < 4 && slot < HASH_GRID_CAPACITY; bucket_offset++)
	{
		...
		if (hash_grid[hash_grid_index].index.x == index.x && hash_grid[hash_grid_index].index.y == index.y) return hash_grid_index;
    atomicAnd(hash_grid[hash_grid_index].active_flags, ~HASH_GRID_LOCKED);
	}
	return -1;
}

ivec2 get_hash_grid_index(vec2 position)
{
	return ivec2(floor(position));
}

bool set_hash_grid_cell(int hash_grid_index, in HashGridCell cell)
{
	const bool is_locked = (hash_grid[hash_grid_index].active_flags & HASH_GRID_LOCKED) != 0;
	const bool is_correct_index = hash_grid[hash_grid_index].index.x == cell.index.x && hash_grid[hash_grid_index].index.y == cell.index.y;
	if (is_locked && is_correct_index)
	{
